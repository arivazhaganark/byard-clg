...
<div class="container spark-screen" style="width:100%;">
    ...
    @endif 
    <div class="row">
        <div class="col-md-8 col-md-offset-2">@if($uPermission[0]->file_add==1) <a class="btn btn-app" href="{{ url('admin/schstaff/add') }}" title="Add"> <i class="fa fa-plus"></i> Add </a> @endif
             @if($uPermission[0]->file_view==1 || $uPermission[0]->file_delete==1 || $uPermission[0]->file_edit==1) 
            <a class="btn btn-app bg-green" href="{{ url('admin/schstaff') }}" title="Active"> <i class="fa fa-check-circle"></i>
                Active ({{ $active_count }}) </a>
            <a class="btn btn-app bg-red" href="{{ url('admin/schstaff/?token=inactive') }}" title="Inactive"> <i class="fa fa-ban"></i>
                ...
                <div class="box-header">
                    <h3 class="box-title">
                        ...
                        ?>">
                               <?php
                               if (isset($_REQUEST['token']) && $_REQUEST['token'] != '') {
                                   echo 'Inactive school staff';
                               } else {
                                   ?>Active school staff<?php } ?>
                        </p>
                    </h3>
                </div>
                @if($StaffAll->count() > 0)
                <div class="box-footer clearfix"> 
                    <form name="frm_action" id="frm_action" method="post" action="{{ url('/admin/schstaff/actionupdate') }}">
                        {{ csrf_field() }}
                    ...
                    <table @if($StaffAll->count()>0) id="example1" @else id="" @endif class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>          
                                ...
                                <td> @if($uPermission[0]->file_edit==1)<a class="btn btn-default" href="<?php echo url('admin/schstaff/edit/'.$StaffAll_data->scl_stf_id); ?>" title="Edit"><i class="fa fa-edit"></i></a>@else --- @endif</td>
                                <td> @if($uPermission[0]->file_delete==1)<input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="{{ $StaffAll_data->scl_stf_id }}">@else --- @endif</td>
                            </tr>
                            <?php $i++; ?>   
                            @endforeach
                            @else
