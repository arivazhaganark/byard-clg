...
<section class="content">
  <?php $category_id = $category->id; $category_name = $category->name; ?>

   <!-- Basic Examples -->
   <div class="row clearfix">
      <ul class="header-dropdown m-r--5">
         <div class="col-md-8 col-md-offset-2">
            <button type="button" class="btn btn-default waves-effect"><a href="{{ url('admin/sub_category/'.$category_id.'/add') }}">
            <i class="material-icons" >add</i></a>
            </button>
            <a class="btn btn-app bg-green" href="{{ url('admin/sub_category/'.$category_id) }}" title="Active"> <i class="material-icons">verified_user</i></i>
            Active ({{ $active_count }}) </a>
            <a class="btn btn-app bg-red" href="{{ url('admin/sub_category/'.$category_id.'/?token=inactive') }}" title="Inactive"> <i class="material-icons">cancel</i>
      ...
      <div class="card">
        ...
        @endif 
         <div class="header">
               ...
               <div class="box-header">
                  <h3 class="box-title">
                     <p class="<?php
                        ...
                        <?php
                           if (isset($_REQUEST['token']) && $_REQUEST['token'] != '') {
                               echo 'Inactive Sub Category "'. $category_name.'"';
                           } else {
                               ?>Active Sub Category "<?php echo $category_name; ?>"<?php } ?>
                     </p>
                  </h3>
               </div>
               @if($sub_category->count() > 0)
               <div class="box-footer clearfix">
                  <form name="frm_action" id="frm_action" method="post" action="{{ url('/admin/sub_category/'.$category_id.'/actionupdate') }}">
                     {{ csrf_field()}}
            ...
            <div class="body">
               <table @if($sub_category->count()>0) id="example1" @else id="" @endif class="table table-bordered table-striped table-hover js-basic-example dataTable">
               <thead>
               ...
               <tbody>
                  <?php $i = 1; ?>
                  @foreach( $sub_category as $category_data )
                  <tr>
                     <td>{{ $i }}</td>
                     <td>{{ $category_data->name }} </td>
                     <td>
                        <a class="btn bg-cyan  btn-xs waves-effect"
                           href="{{url('admin/sub_category/'.$category_id.'/edit', $category_data->id) }}" title="Edit"><i class=" material-icons" style="color:green">edit</i></a>
                       </td>
                     <td>
