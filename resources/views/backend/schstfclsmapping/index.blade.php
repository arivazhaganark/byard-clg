...
<div class="container spark-screen" style="width:100%;">
    ...
    @endif 
    <div class="row">
        <div class="col-md-8 col-md-offset-2">@if($uPermission[0]->file_add==1) <a class="btn btn-app" href="{{ url('admin/schstfclsmap/add') }}" title="Add"> <i class="fa fa-plus"></i> Add </a>@endif
         @if($uPermission[0]->file_view==1 || $uPermission[0]->file_delete==1 || $uPermission[0]->file_edit==1) 
            <a class="btn btn-app bg-green" href="{{ url('admin/schstfclsmap') }}" title="Active"> <i class="fa fa-check-circle"></i>
                Active ({{ $active_count }}) </a>
            <a class="btn btn-app bg-red" href="{{ url('admin/schstfclsmap/?token=inactive') }}" title="Inactive"> <i class="fa fa-ban"></i>
        ...
        <div class="col-xs-12">
            ...
            ?>">
                ...
                <div class="box-footer clearfix"> 
                    <form name="frm_action" id="frm_action" method="post" action="{{ url('/admin/schstfclsmap/actionupdate') }}">
                        {{ csrf_field() }}
                        ...
                        <tbody>
                            ...
                            <tr>          
                                <td>{{ $i }}</td>                        
                                 ...
                                 <td><!-- <a class="btn btn-default" href="<?php  //echo url('admin/schstfclsmap/edit/'.$schSubjectAll_data->sch_cls_stf_id); ?>" title="Edit"><i class="fa fa-edit"></i></a>  &nbsp;&nbsp; -->@if($uPermission[0]->file_edit==1)<a class="btn btn-default" href="<?php  echo url('admin/schstfclsmap/show/'.$schSubjectAll_data->sch_cls_stf_id); ?>" title="View"><i class="fa fa-file-text"></i></a> @else --- @endif    </td> 
                                <td>@if($uPermission[0]->file_delete==1)<input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="{{ $schSubjectAll_data->sch_cls_stf_id }}">@else --- @endif</td>
                            </tr>
                            <?php $i++; ?>   
                            @endforeach
                            @else
