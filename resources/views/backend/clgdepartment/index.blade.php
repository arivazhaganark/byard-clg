...
<div class="container spark-screen" style="width:100%;">
    ...
    <div class="row">
        ...
        @if($uPermission[0]->file_add==1)
         <a class="btn btn-app" href="{{ url('admin/clgdepart/add') }}" title="Add"> <i class="fa fa-plus"></i> Add </a>
        @endif

         @if($uPermission[0]->file_view==1 || $uPermission[0]->file_delete==1 || $uPermission[0]->file_edit==1)
         <a class="btn btn-app bg-green" href="{{ url('admin/clgdepart') }}" title="Active"> <i class="fa fa-check-circle"></i>
                Active ({{ $active_count }}) </a>
            <a class="btn btn-app bg-red" href="{{ url('admin/clgdepart/?token=inactive') }}" title="Inactive"> <i class="fa fa-ban"></i>
                ...
                <div class="box-header">
                    <h3 class="box-title">
                        ...
                        ?>">
                               <?php
                               if (isset($_REQUEST['token']) && $_REQUEST['token'] != '') {
                                   echo 'Inactive Department';
                               } else {
                                   ?>Active Department<?php } ?>
                        </p>
                    </h3>
                </div>
                @if($clgdepartAll->count() > 0)
                <div class="box-footer clearfix"> 
                    <form name="frm_action" id="frm_action" method="post" action="{{ url('/admin/clgdepart/actionupdate') }}">
                        {{ csrf_field() }}
                        ...
                        <tbody>
                            ...
                            <tr>          
                                ...
                                <td>{{ $clgdepartAll_data->grad_name }} </td> 
                                   ...
                                   <td>
                                  <a class="btn btn-default" href="<?php  echo url('admin/clgdepart/edit/'.$clgdepartAll_data->dep_id); ?>" title="Edit"><i class="fa fa-edit"></i></a>  </td>  
                                  @else 
                                  <td>---</td>  
                                 @endif
                                  @if($uPermission[0]->file_delete==1)
                                 <td><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="{{ $clgdepartAll_data->dep_id }}"></td>
                                 @else
                                 <td>---</td>
                                 @endif
