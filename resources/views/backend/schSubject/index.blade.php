...
<div class="container spark-screen" style="width:100%;">
    ...
    @endif 
    <div class="row">
        <div class="col-md-8 col-md-offset-2">@if($uPermission[0]->file_add==1) <a class="btn btn-app" href="{{ url('admin/schsubject/add') }}" title="Add"> <i class="fa fa-plus"></i> Add </a>@endif
        @if($uPermission[0]->file_view==1 || $uPermission[0]->file_delete==1 || $uPermission[0]->file_edit==1) 
            <a class="btn btn-app bg-green" href="{{ url('admin/schsubject') }}" title="Active"> <i class="fa fa-check-circle"></i>
                Active ({{ $active_count }}) </a>
            <a class="btn btn-app bg-red" href="{{ url('admin/schsubject/?token=inactive') }}" title="Inactive"> <i class="fa fa-ban"></i>
        ...
        <div class="col-xs-12">
            ...
            ?>">
                ...
                <div class="box-footer clearfix"> 
                    <form name="frm_action" id="frm_action" method="post" action="{{ url('/admin/schsubject/actionupdate') }}">
                        {{ csrf_field() }}
                ...
                <div class="box-body">
                    <table @if($schSubjectAll->count()>0) id="example1" @else id="" @endif class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>          
                                <td>{{ $i }}</td>                        
                                <td>{{ $schSubjectAll_data->sch_class }} </td>  
                                          <td>{{ $schSubjectAll_data->sub_name }} </td>  
                                 <td> @if($uPermission[0]->file_edit==1)<a class="btn btn-default" href="<?php  echo url('admin/schsubject/edit/'.$schSubjectAll_data->sub_id); ?>" title="Edit"><i class="fa fa-edit"></i></a>@else --- @endif   </td> 
                                <td> @if($uPermission[0]->file_delete==1)<input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="{{ $schSubjectAll_data->sub_id }}">@else --- @endif</td>
                            </tr>
                            <?php $i++; ?>   
                            @endforeach
                            @else
