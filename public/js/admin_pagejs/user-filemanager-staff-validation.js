 ...
 function ajaxfm(pwd=null, file_id=null, search_txt=null,folderName=null) { 
     ...
     if(fileid>0)
     {
      ...
      slinkPathArr.push(valSplitIdVal);
       ...
       if(slinkPathArr.length>0)
       {
        ...
        $.ajax({
        type: "POST",
        url: app_url + "/user/filemanager/ajaxsearchfm",
    ...
    function clsLink(a)
    {
     ...
     function delete_file(cFileName=null,cFileIs=null,cFiledelPath=null) {
      ...
      $.ajax({
          type: "POST",
          url: app_url + "/user/filemanager/delete",
   ...
   $('#search_txt').keyup(function(){ 
             ...
             $('#searchhidVal').val("yes");

         $('#brdcrum').html('<a class="root" href='+app_url+'/user/filemanager'+'>Home</a>&nbsp;&raquo;&nbsp;Search result');    
   ...
   function createfolder(folderId=null,lkPathId=null) { 
       ...
       $.ajax({
           type: "POST",
           url: app_url + "/user/filemanager/create_folder",
...
$('#brdcrum').html("");
var bPushVal='<a class="root" href='+app_url+'/user/filemanager'+'>Home</a>&nbsp;&raquo;&nbsp;';
         ...
         var valSplitId=brray[i].split('_')[1];
          ...
          bPushVal+="<span class='root' onclick='return fnTs1("+valSplitId+")'  >"+valSplitTxt+"</span>&nbsp;&raquo;&nbsp;";
           ...
           var searchYes=$('#searchhidVal').val(); 
           if(searchYes=="yes")
           {
              $('#brdcrum').html('<a class="root" href='+app_url+'/user/filemanager'+'>Home</a>&nbsp;&raquo;&nbsp;Search result');
           }
  ...
  return this.reduce(function(previous, current, index, array)
   {
     ...
     function fnTsSearch(rId,pathVal)
     {
        $('.flderLinks').prop('onclick',null).off('click'); 
        $('#crbtn,#upBtn,#btnRefresh').attr('lang',rId);
        $('#brdcrum').html('<a class="root" href='+app_url+'/user/filemanager'+'>Home</a>&nbsp;&raquo;&nbsp;Search result');
        brray=[];
        var expVal=pathVal.split(',');
       
        for(var ii=0;ii<expVal.length;ii++) 
        {
          brray.push(expVal[ii]); 
        }
        ajaxfm('', rId, '','');
    ...
    //  alert(new_value);
     // alert(file_id)
      if(new_value !='') {
          $.ajax({
              type: "POST",
              url: app_url + "/user/filemanager/rename",
...
function rename_folder(e) { 
     ...
     // alert(file_id)
      if(new_value !='') {
        ...
        if(/^[a-zA-Z0-9.-]+$/.test($.trim(new_value))) {
          $.ajax({
              type: "POST",
              url: app_url + "/user/filemanager/renameflder",
   ...
   function getBrdcrums(ids)
   {
    ...
    $.ajax({
           type: "POST",
           url: app_url + "/user/filemanager/getBrdcrum",
   ...
   function publish_file(pFileId=null,pFileIdMode=null) {
       $.ajax({
              type: "POST",
              url: app_url + "/user/filemanager/filePublish",
...
Dropzone.options.imageUpload = {
           url: app_url +"/user/filemanager/create_file",  
           maxFilesize: 10, //MB
