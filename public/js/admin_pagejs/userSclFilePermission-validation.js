...
$('#sUser').on('change', function() {
   ...
   $('#errMsg').html("");
    if(selectUsrVal>0)
    {
         ...
         $.ajax({  
                type: "POST",
                url : app_url+'/admin/userSclFile/userPermissionAjax',   
                    ...
                    if(data.status==1)
                    {
                        ...
                        if(data.output.length>0)
                        {
                            for(var i=0;i<data.output.length;i++)
                            {
                                var addEveNo=i%2;
                                var rowClass='odd'
                                var AddId='add_'+data.output[i].m_s_id;
                                var DelId='del_'+data.output[i].m_s_id;
                                var EditId='edit_'+data.output[i].m_s_id;
                                var ViewId='view_'+data.output[i].m_s_id;
                                var AddBox="";
                                var EditBox="";
                                var DelBox="";
                                var ViewBox="";
                                ...
                                innTr+="<tr class="+rowClass+" role='row'><td>"+(i+1)+"</td><td>"+data.output[i].menu_name+"</td><td><input id='"+AddId+"' class='chkall' name='"+AddId+"' "+AddBox+" value='"+data.output[i].m_s_id+"' type='checkbox'></td><td><input id='"+EditId+"' class='chkall' "+EditBox+" name='"+EditId+"' value='"+data.output[i].m_s_id+"' type='checkbox'></td><td><input id='"+ViewId+"' class='chkall' "+ViewBox+" name='"+ViewId+"' value='"+data.output[i].m_s_id+"' type='checkbox'></td><td><input id='"+DelId+"' class='chkall' "+DelBox+" name='"+DelId+"' value='"+data.output[i].m_s_id+"' type='checkbox'></td><tr>"
                            }
                        }
                       ...
                       $.ajax({  
                        type: "POST",
                        url : app_url+'/admin/userSclFile/userPermissionCurdAjax',   
